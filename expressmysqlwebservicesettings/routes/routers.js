...
myrouter.post("/products/product/:pid",function(req,resp){
    ...
    connection.query("insert into product values(?,?,?,?)",[req.body.pid,req.body.pname,req.body.price,req.body.qty],(err,result)=>{
        if(err){
...
myrouter.put("/products/product/:pid",function(req,resp){
    connection.query("update product set pname=?,qty=?,price=? where pid=?",[req.body.pname,req.body.price,req.body.qty,req.body.pid],(err,result)=>{
        if(err){
            resp.status(500).send("no data added");
        }else{
            console.log(result);
